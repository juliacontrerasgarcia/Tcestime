...
def print_tc(net_val, h_frac, h_dos, molec=None, fit='leastsq', code='QE'):
    t_c, eq_tc = get_tc(net_val, h_frac, h_dos, molec, fit, code)
...
def error_vasp_fit(fit):
    return (
        f"\033[91m[ERROR]\033[0m For VASP calculations, only the neural-network model (--fit NN) is supported.\n"
        f"This execution was aborted because the selected fit ('{fit}') is incompatible with VASP.\n"
        "Please rerun the command as:\n"
        "  python TcESTIME.py --code VASP --fit NN ..."
    )
